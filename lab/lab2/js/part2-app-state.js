...
var makeMarkers = function(data) {
  ...
  for(i = 0; i < data.length; i++){
    var marker = L.marker([data[i].CapitalLatitude,data[i].CapitalLongitude]);
    collection.push(marker) ;
  }
  return collection;

//return _.map(data, function(each){return L.marker([each.CapitalLatitude, each.CapitalLongitude]);});
